...
class CriticAgentV3(AgentV3):
    ...
    # The critic's final JSON (corrections + claims + citations across 6 agents)
    # routinely exceeds the V3 default 4096-token cap and truncated mid-string,
    # parsing to nothing. 8192 is the V3 maximum.
    ...
    def execute(
        ...
        # Provided to the model as CONTEXT only. They are merged back into the
        # findings in Python after the loop — forcing the LLM to echo them
        # verbatim wasted output tokens and contributed to JSON truncation.
...
- `citations` — list of structured provenance objects when the fact is grounded in a document:
    [{{"doc_id": "<source file or ChromaDB doc id>",
       ...
       "snippet": "<up to 120 chars of the exact source text — HARD LIMIT, truncate longer quotes>",
       "page": <int or null>}}]
...
- Do NOT include a `narrative_inconsistencies` key in your output — known
  inconsistencies are attached automatically by the system after your review.

## NARRATIVE INCONSISTENCIES ALREADY ON RECORD (context only — do NOT echo)
Detected across prior fiscal periods by the institutional memory layer. Use them
to inform your verification (e.g. heightened skepticism on drifting metrics),
but do NOT reproduce them in your output:
...
Output your findings as JSON matching the schema in your instructions."""
        ...
        # ── Re-attach known inconsistencies (deterministic, not LLM-echoed) ──
        # PM Synthesis and the UI read these from the critic's findings; doing
        # the merge here guarantees they survive regardless of output size.
        if react_result.final_output is not None:
            react_result.final_output["narrative_inconsistencies"] = known_inconsistencies
