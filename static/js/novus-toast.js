window.showToast = function(message, type = 'error') {
    ...
    toast.className = `px-4 py-3 rounded shadow-lg flex items-center gap-3 transition-all duration-300 transform translate-y-4 opacity-0 text-sm font-sans font-medium text-white ${
        type === 'error' ? 'bg-semantic-red border-l-4 border-white' : 
        type === 'success' ? 'bg-semantic-green border-l-4 border-white' : 
        'bg-base-elevated border-l-4 border-accent-brand'
    }`;
